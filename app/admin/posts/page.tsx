    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link href="/admin/posts/new" className="rounded-full bg-[#1A5C38] px-4 py-1.5 text-sm font-semibold text-white shadow-sm hover:bg-emerald-800">
            + New Post
          </Link>
        </div>
      </div>
        ...
        <table className="min-w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-gray-100">
            {loading ? (
              ...
              <tr key={post._id} className={`hover:bg-gray-50 ${selected.has(post._id) ? 'bg-red-50/40' : ''}`}>
                ...
                <td className="px-4 py-2 text-gray-700">{post.createdAt ? format(new Date(post.createdAt), 'dd MMM yyyy') : '-'}</td>
                <td className="px-4 py-2 text-right">
                  <Link href={`/admin/posts/${post._id}/edit`} className="mr-3 text-[#1A5C38] hover:underline">Edit</Link>
