...
export default function AdminRecipesPage() {
  ...
  const fetchRecipes = useCallback(async () => {
    setLoading(true)
    const res = await fetch(`/api/admin/recipes?pageSize=200`)
  ...
  const handleBulkDelete = async () => {
    ...
    setAllRecipes((prev) => prev.filter((r) => !selected.has(r._id)))
    setSelected(new Set())
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">{allRecipes.length} recipe{allRecipes.length !== 1 ? 's' : ''}</p>
        </div>
        ...
        <table className="min-w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-gray-50">
            {loading ? (
              ...
              <tr key={r._id} className={`transition-colors hover:bg-gray-50/60 ${selected.has(r._id) ? 'bg-red-50/40' : ''}`}>
                ...
                <td className="px-4 py-3">
                  ...
                  <p className="text-[11px] text-gray-400 truncate max-w-[220px]">{r.slug}</p>
                </td>
                <td className="px-4 py-3">
                  <span className={`inline-flex rounded-full px-2 py-0.5 text-[11px] font-bold ${LANG_BADGE[r.language ?? 'en'] ?? 'bg-gray-100 text-gray-600'}`}>
                    {LANG_LABEL[r.language ?? 'en'] ?? r.language}
                  </span>
                </td>
                <td className="px-4 py-3 text-sm capitalize text-gray-600">{r.category ?? '—'}</td>
                <td className="px-4 py-3"><StatusPill published={r.isPublished} /></td>
                <td className="px-4 py-3 text-sm text-gray-500">{r.createdAt ? format(new Date(r.createdAt), 'dd MMM yyyy') : '—'}</td>
                <td className="px-4 py-3 text-right">
                  <div className="flex items-center justify-end gap-2">
                    <Link href={`/admin/recipes/${r._id}/edit`}
                    ...
                    <button type="button" disabled={deleting === r._id} onClick={() => handleDelete(r._id, r.title)}
      ...
      <div className="space-y-3 md:hidden">
        {loading ? (
          ...
          <div key={r._id}
            className={`rounded-2xl border bg-white p-4 shadow-sm transition ${selected.has(r._id) ? 'border-red-300 bg-red-50/20' : 'border-gray-100'}`}>
            <div className="flex items-start gap-3">
              <input type="checkbox" checked={selected.has(r._id)} onChange={() => toggleOne(r._id)}
                className="mt-0.5 h-4 w-4 rounded border-gray-300 text-[#1A5C38] focus:ring-[#1A5C38]" />
              <div className="min-w-0 flex-1">
                <p className="font-semibold text-gray-900 leading-snug line-clamp-2">{r.title}</p>
                <p className="mt-0.5 text-[11px] text-gray-400 truncate">{r.slug}</p>
                <div className="mt-2 flex flex-wrap items-center gap-1.5">
                  <span className={`inline-flex rounded-full px-2 py-0.5 text-[10px] font-bold ${LANG_BADGE[r.language ?? 'en'] ?? 'bg-gray-100 text-gray-600'}`}>
                    {LANG_LABEL[r.language ?? 'en'] ?? r.language}
                  </span>
                  <StatusPill published={r.isPublished} />
                  {r.category && <span className="text-[11px] capitalize text-gray-500">{r.category}</span>}
                  <span className="text-[11px] text-gray-400">{r.createdAt ? format(new Date(r.createdAt), 'dd MMM yyyy') : '—'}</span>
                </div>
              </div>
            </div>
            <div className="mt-3 flex gap-2 border-t border-gray-100 pt-3">
              ...
              <button type="button" disabled={deleting === r._id} onClick={() => handleDelete(r._id, r.title)}
                ...
                <Trash2 size={12} /> {deleting === r._id ? '…' : 'Delete'}
              </button>
            </div>
          </div>
        ))}
